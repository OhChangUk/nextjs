...
export default function Work(){
    ...
    const menuArray = ["전체", "클론", "팀 프로젝트", "웹앱"]
        ...
        <>
            <div className="max-w-7xl mx-auto mt-8">
                    ...
                    </ul>
                    {
                        menuType.map((e,i)=>{
                            return(
                                <p key={i} onClick={()=>setIsActive(i)} className={`${isActive === i ? "block" : "hidden"}`}>{e}</p>
                            )
                        })
                    }
                    <div className="mt-8">
                        {
                            FilterData.map((e,i)=>{
                                return(
                                    <div onClick={()=>setIsActive(i)} className={`bg-white mb-8 pt-12 group px-8 pb-16 rounded-md flex border flex-wrap`} key={i}>
                                        <div className="basis-[48%] relative mokup-img group-even:order-1 xl:group-even:order-2">
                                            <div>
                                                <Image width={500} height={500} src="/images/mokup-1.png" alt="1" />
                                            </div>
                                            <div>
                                                <Image width={500} height={500} src="/images/mokup-2.png" alt="1" />
                                            </div>
                                            <div>
                                                <Image width={500} height={500} src="/images/mokup-3.png" alt="1" />
                                            </div>
                                        </div>
                                        <div className="basis-[52%] group-even:order-2 xl:group-even:order-1 pt-10">
                                            ...
                                            <ul className="flex justify-center mt-6">
                                                ...
                                                <li className="py-1 px-8 border rounded-md text-sm mr-4"><a href={e.original} target="_blank">Create</a></li>
                                                <li className="py-1 px-8 border rounded-md text-sm"><a href={e.original} target="_blank">Git</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                )
