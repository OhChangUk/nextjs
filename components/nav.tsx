    ...
    return(
        <>
            ...
            <div onClick={toggleMenu} className="fixed right-5 top-5 transition-all duration-500 z-[51] cursor-pointer md:hidden">
                <div className={`w-[30px] h-[5px] bg-black dark:db-[#ebf4f1] rounded m-[5px] transition-all duration-500 ${menuActive && 'rotate-45 translate-y-[10px]'}`}></div>
                <div className={`w-[30px] h-[5px] bg-black dark:db-[#ebf4f1] rounded m-[5px] transition-all duration-500 ${menuActive && 'opacity-0 -translate-x-1'}`}></div>
                <div className={`w-[30px] h-[5px] bg-black dark:db-[#ebf4f1] rounded m-[5px] transition-all duration-500 ${menuActive && '-rotate-45 -translate-y-[10px]'}`}></div>
            </div>
                ...
                <ul className="mt-12">
                    {
                        menuList.map((e,i)=>{
                            return(
                                <li className="pt-5 pb-2 border-b hover:font-bold" key={i}><FontAwesomeIcon icon={icons[i]} className="mr-2" />
                                <Link href={e.link}>{e.name}</Link>
                                </li>
                            )
                        })
